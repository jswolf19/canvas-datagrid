...
var isPrintableKeyEvent = require('is-printable-key-event');
...
define([], function () {
    ...
    return function (self) {
        ...
        self.pasteData = function(pasteValue, mimeType, startRowIndex, startColIndex) {
            ...
            function isSupportedHtmlTable(htmlString) { return /(?:^(<meta[^>]+>)?<table>)|(?:<!--StartFragment-->\s*<table>)/.test(htmlString) };
            ...
            function parseData(data, mimeType) {
                ...
                if (mimeType === "text/html") {
                    ...
                    var cleanedHtmlData = data.substring(
                        ...
                        function(row) { return !/^<th>|^<thead>/.test(row) }
                    ).map(
                        // split row into individual cells
                        function(row) { return row.match(/<td>[^<]+/g).map(function(match) { return match.replace(/^<td>/, '') }) }
                    );
        ...
        self.paste = function (event) {
            ...
            var clipboardItems = new Map(
                Array.from(event.clipboardData.items).map(function(item) { return [item.type, item] })
            );
            ...
            var pasteableItems = supportedMimeTypes.map(
                function(mimeType) { return clipboardItems.get(mimeType) }
            ).filter(function(item) { return !!item }); // filter out not-found MIME types (= undefined)
